...
const ERPPurchaseOrderForm = () => {
  ...
  const fetchSuppliers = useCallback(async () => {
    try {
      // 暫時使用硬編碼數據，因為後端沒有 /api/suppliers 端點
      setSuppliers([
        { supplierId: 1, name: "供應商 A" },
        { supplierId: 2, name: "供應商 B" },
        { supplierId: 3, name: "供應商 C" }
      ]);
    } catch (error) {
  ...
  const fetchWarehouses = useCallback(async () => {
    try {
      // 暫時使用硬編碼數據，因為後端沒有 /api/warehouses 端點
      setWarehouses([
        { warehouseId: 1, name: "台北總倉" },
        { warehouseId: 2, name: "台中冷凍倉" },
        { warehouseId: 3, name: "高雄物流中心" }
      ]);
    } catch (error) {
  ...
  const fetchPurchaseOrder = useCallback(async (orderId) => {
    try {
      ...
      const mappedDetails = order.details?.map((detail, index) => ({
        key: index,
        id: detail.id,
  ...
  const renderStatusTag = () => {
    ...
    const config = statusConfig[purchaseOrder.status] || { color: 'gray', text: '未知' };
    
    return (
      <span style={{ 
        padding: '4px 8px', 
        backgroundColor: config.color === 'default' ? '#f5f5f5' : 
                          config.color === 'blue' ? '#e6f7ff' :
                          config.color === 'orange' ? '#fff7e6' :
                          config.color === 'green' ? '#f6ffed' :
                          config.color === 'red' ? '#fff2f0' : '#fafafa',
        color: config.color === 'default' ? '#666' :
               config.color === 'blue' ? '#1890ff' :
               config.color === 'orange' ? '#fa8c16' :
               config.color === 'green' ? '#52c41a' :
               config.color === 'red' ? '#ff4d4f' : '#666',
        borderRadius: '4px',
        fontSize: '12px',
        fontWeight: 'bold'
      }}>
  ...
  const detailColumns = [
    {
      ...
      render: (value, record) => (
        <Select
          ...
          disabled={purchaseOrder?.status === 'CONFIRMED' || purchaseOrder?.status === 'RECEIVED'}
        >
      ...
      render: (_, record) => (
        ...
        >
          <Button 
            ...
            disabled={details.length === 1 || purchaseOrder?.status === 'CONFIRMED' || purchaseOrder?.status === 'RECEIVED'}
          />
        </Popconfirm>
      ),
    },
      ...
      >
        ...
        >
          <Row gutter={16}>
            ...
            <Col span={8}>
              <Form.Item
                ...
                name="currency"
              >
                <Select disabled={purchaseOrder?.status === 'CONFIRMED' || purchaseOrder?.status === 'RECEIVED'}>
          ...
          <Card 
            title="進貨明細" 
            extra={
              (!purchaseOrder || (purchaseOrder.status !== 'CONFIRMED' && purchaseOrder.status !== 'RECEIVED')) && (
                <Button 
                  type="dashed" 
                  onClick={addDetail}
                  icon={<PlusOutlined />}
          ...
          >
            ...
            <div className="mt-4 text-right">
              <Space size="large">
                <span>總計：<strong>${calculateTotal().toLocaleString()}</strong></span>
              </Space>
            </div>
          </Card>
