...
const Navbar = () => {
  ...
  return (
    <>
      {/* 導覽列本體 */}
      <nav
        className={` transition duration-300 px-6 py-4 backdrop-blur-sm ${
          isHomeAtTop
            ? "bg-white/10 text-logo-tan shadow-xs"
            : "bg-logo-blue/95 text-logo-tan shadow-md"
        }`}
      >
        <div className="container mx-auto flex justify-between items-center">
          ...
          <div className="hidden md:flex items-center space-x-6 font-semibold text-lg">
            ...
            <Link to="/news" className="hover:text-logo-lightBlue">
              最新消息
            </Link>
            <Link to="/contact" className="hover:text-logo-lightBlue">
              聯絡我們
            </Link>
            <Link to="/login" className="text-2xl hover:text-logo-lightBlue">
              <FaUserCircle />
            </Link>
            <button  onClick={toggleCart}>
            <FaShoppingCart className="text-2xl hover:text-logo-lightBlue cursor-pointer" />
            {cartItemCount > 0 && (
      ...
      >
        ...
        {/* 側邊滑出選單 */}
        <div
          className={` z-[46] absolute top-0 left-0 h-full w-72 bg-logo-blue text-logo-tan transform transition-transform duration-300 ease-in-out ${
