...
public class DemoApplication {
	...
	public CommandLineRunner demoDataSeeder(BCustomerRepository BCustomerRepository,
		...
		return args -> {
			...
			BCustomerIndustry[] industries = BCustomerIndustry.values();
			BCustomerType[] BCustomerTypes = BCustomerType.values();
			BCustomerLevel[] BCustomerLevels = BCustomerLevel.values();
			...
			if (BCustomerRepository.count() == 0) { // 使用 count() 方法判斷表是否為空
				for (int i = 0; i < 50; i++) { // 生成 50 個假客戶
					BCustomer BCustomer = new BCustomer();
					BCustomer.setCustomerName(faker.company().name()); // 公司名稱
					BCustomer.setCustomerEmail(faker.internet().emailAddress()); // 電子郵件
					BCustomer.setCustomerTel(faker.phoneNumber().phoneNumber()); // 電話號碼
					BCustomer.setCustomerAddress(faker.address().fullAddress()); // 完整地址

					// 隨機選擇 Enum 值
					BCustomer.setIndustry(industries[random.nextInt(industries.length)]);
					BCustomer.setBCustomerType(BCustomerTypes[random.nextInt(BCustomerTypes.length)]);
					BCustomer.setBCustomerLevel(BCustomerLevels[random.nextInt(BCustomerLevels.length)]);

					BCustomerRepository.save(BCustomer); // 保存到資料庫
					// 為每個客戶生成 1-3 個聯絡人
					...
					for (int j = 0; j < numberOfContacts; j++) {
						...
						BCustomer.addContact(contact); // 這會自動設定 contact.setCustomer(customer);
						...
						System.out.println("  > 生成聯絡人: " + contact.getContactName() + " (屬於客戶: " + BCustomer.getCustomerName() + ")");
					}
					...
					BCustomerRepository.save(BCustomer); // 保存客戶和所有關聯的聯絡人
					System.out.println("生成客戶: " + BCustomer.getCustomerName() + " 完成。");
				}
			} else {
